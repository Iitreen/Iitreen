...
The Litreen Core daemon can be configured to act as a trusted "border
router", implementing the litreen wire protocol and relay, making
...
The ZeroMQ feature in Litreen Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building litreend:
...
For instance:
    ...
    $ litreend -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/litreend.tx.raw \
...
These options can also be provided in litreen.conf.
...
From the perspective of litreend, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into litreend directly. Furthermore, no information is
...
using. Litreend appends an up-counting sequence number to each
