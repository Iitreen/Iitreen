Litreen Core version 0.18.1 is now available from:

  <https://download.litreen.org/litreen-0.18.1/>
...
`/Applications/Litreen-Qt` (on Mac) or `litreend`/`litreen-qt` (on
Linux).
...
Litreen Core is supported and extensively tested on operating systems
using the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not
recommended to use Litreen Core on unsupported systems.
...
10.10. Additionally, Litreen Core does not yet change appearance when
...
- The `rpcallowip` option can no longer be used to automatically listen
  ...
  [...] -p 127.0.0.1:16647:16647` (this is an extra `:16647` over the
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](mailto:contact@litreen.org) the Litreen Core
  project to let us know about their use-case, as this feature could be
  deprecated in the future).

Wallet changes
--------------
When creating a transaction with a fee above `-maxtxfee` (default 0.1 LTRN),
the RPC commands `walletcreatefundedpsbt` and  `fundrawtransaction` will now fail
instead of rounding down the fee. Beware that the `feeRate` argument is specified
in LTRN per kilobyte, not litoshi per byte.
...
- A new [document](/blob/master/doc/litreen-conf.md)
  about the `litreen.conf` file describes how to use it to configure
  Litreen Core.

- A new document introduces Litreen Core's BIP174 [Partially-Signed
  Litreen Transactions
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent Litreen-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  Litreen Core to libssl vulnerabilities in the past, builders who don't
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when Litreen Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of Litreen Core, subsequently importing
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart litreend with the `-deprecatedrpc=generate`
...
- A new `litreen-wallet` tool is now distributed alongside Litreen
...
This section describes planned changes to Litreen Core that may affect
other Litreen software and services.

- Since version 0.16.0, Litreen Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with Litreen Core 0.20 (expected about
  a year after 0.18), Litreen Core will default to native segwit
  ...
  to bech32 addresses, and if the Litreen Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in Litreen Core 0.20.
  ...
  set the `addresstype=bech32` configuration option in any Litreen Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, Litreen Core
...
- A sub-project of Litreen Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with Litreen Core.  See their [project
...
- This release changes the Random Number Generator (RNG) used from
  OpenSSL to Litreen Core's own implementation, although entropy
  gathered by Litreen Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves Litreen Core a
...
- On macOS, Litreen Core now opts out of application CPU throttling
...
- #16647 Let wallet `importmulti` RPC accept labels for standard scriptPubKeys (ryanofsky)
